...
public class DegradeController {
    ...
    private boolean publishRules(String app, String ip, Integer port) {
        ...
        try {
			...
			logger.info("添加降级规则成功{}",JSON.toJSONString(rules.stream().map(DegradeRuleEntity::toDegradeRule).collect(Collectors.toList())));
			return true;
		} catch (Exception e) {
			logger.info("添加降级规则失败{}",JSON.toJSONString(rules.stream().map(DegradeRuleEntity::toDegradeRule).collect(Collectors.toList())));
			e.printStackTrace();
			return false;
		}
    }
