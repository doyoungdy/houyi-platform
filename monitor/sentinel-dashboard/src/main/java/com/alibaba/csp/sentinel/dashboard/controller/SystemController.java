...
public class SystemController {
    ...
    private boolean publishRules(String app, String ip, Integer port) {
        ...
        try {
			...
			logger.info("添加系统规则成功{}",JSON.toJSONString(rules.stream().map(SystemRuleEntity::toSystemRule).collect(Collectors.toList())));
			return true;
		} catch (Exception e) {
			logger.info("添加系统规则失败{}",JSON.toJSONString(rules.stream().map(SystemRuleEntity::toSystemRule).collect(Collectors.toList())));
			e.printStackTrace();
			return false;
		}
    }
