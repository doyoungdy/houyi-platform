package com.houyi.oauth.controller;
...
import com.houyi.common.utils.SpringUtil;
import com.houyi.oauth.mobile.MobileAuthenticationToken;
...
import com.houyi.oauth2.common.token.OpenIdAuthenticationToken;
...
public class OAuth2Controller {
    ...
    @ApiOperation(value = "客户端模式获取token")
    @PostMapping(SecurityConstants.CLIENTID_TOKEN_URL)
    public void getTokenByClientId(HttpServletRequest request, HttpServletResponse response) throws IOException {
        writerToken(request, response, null, "账户或密码错误");
    }
    ...
    private void writerToken(HttpServletRequest request, HttpServletResponse response, AbstractAuthenticationToken token
        ...
        try {
            ...
            //token为null时默认是客户端请求的模式
            if(token == null){
            	token = new ClientAuthenticationToken(clientId);
            }
